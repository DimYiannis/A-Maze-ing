...
def fill_rect(buf: memoryview, x0: int, y0: int, x1: int, y1: int,
...
def blit_tile(buf: memoryview, tile: bytearray,
    ...
    """
        ...
        args:
            ...
            dx:      Destination x in pixels (can be negative if tile is off-screen left).
            dy:      Destination y in pixels (can be negative if tile is off-screen top).
...
class MazeDisplay:
    ...
    def ensure_cache(self) -> None:
        ...
        scaled = base if tile_px == TILE_SIZE else {
            hv: scale_tile(t, tile_px) for hv, t in base.items()
        }
    ...
    def render(self) -> None:
        ...
        self.draw_portal(ec*tile_px+self.offset_x+half,
                          er*tile_px+self.offset_y+half,
                          ENTRY_COLOR, sl, max_y)
        self.draw_portal(xc*tile_px+self.offset_x+half,
    ...
    def draw_portal(self, cx: int, cy: int,
        ...
        for dy2 in range(-14, 15):
            for dx2 in range(-14, 15):
                if dx2*dx2 + dy2*dy2 <= 14*14:
                    fill_rect(buf, cx+dx2, cy+dy2, cx+dx2+1, cy+dy2+1,
                           r, g, b, sl, max_y)
